...
export default function TechnicalDetails(): JSX.Element {
  ...
  const { localSettings, isLoading, isSaving, handleSave, updateSection, isFormChanged } =
    useTechnicalSettings();

  if (!localSettings) return <LoadingSpinner message="Technical Details Loading" />;

  return (
    <div className="flex-1 space-y-6 p-1 md:p-8">
      <div className="flex flex-col space-y-2">
        <h1 className="text-3xl font-bold tracking-tight">{translation("Technical Details")}</h1>
        <p className="text-muted-foreground">
          {translation("Configure server technical settings, capacities, and file sharing limits")}.
        </p>
      </div>

      <Tabs defaultValue="authentication" className="w-full">
        <TabsList className="grid w-full grid-cols-3 mb-6 truncate">
          <TabsTrigger className="max-sm:text-xs truncate" value="authentication">
